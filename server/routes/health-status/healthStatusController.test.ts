...
describe('getView', () => {
  it('Happy path with default query params and data from endpoints', async () => {
    ...
    await request(app)
      .get('/health-status?prisonerNumber=123')
      .expect(200)
      .expect(res => expect(res.text).toMatchSnapshot())

    expect(getCrsReferralsSpy).toHaveBeenCalledWith('123', 'HEALTH')
    expect(getAssessmentInformationSpy).toHaveBeenCalledWith('123', 'HEALTH')
    ...
    expect(getCaseNotesCreatorsSpy).toHaveBeenCalledWith('123', 'HEALTH')
  })
  ...
  it('Happy path with specified query params and data from endpoints', async () => {
    ...
    await request(app)
      .get(
        '/health-status?prisonerNumber=123&page=1&pageSize=20&sort=occurenceDateTime%2CASC&days=30&createdByUserId=2',
      )
      .expect(200)
      .expect(res => expect(res.text).toMatchSnapshot())
    ...
    expect(getCrsReferralsSpy).toHaveBeenCalledWith('123', 'HEALTH')
    expect(getAssessmentInformationSpy).toHaveBeenCalledWith('123', 'HEALTH')
    expect(getCaseNotesHistorySpy).toHaveBeenCalledWith('123', 'HEALTH', '2', '20', '1', 'occurenceDateTime,ASC', '30')
    expect(getCaseNotesCreatorsSpy).toHaveBeenCalledWith('123', 'HEALTH')
  })
