...
describe('getView', () => {
  ...
  it('Happy path with specified query params and data from endpoints', async () => {
    ...
    await request(app)
      .get(
        '/accommodation?prisonerNumber=A1234DY&page=1&pageSize=20&sort=occurenceDateTime%2CASC&days=30&createdByUserId=2&supportNeedUpdateFilter=1456&supportNeedUpdateSort=createdDate,ASC',
      )
      .expect(200)
      .expect(res => expect(res.text).toMatchSnapshot())
